use crate::{addr_utils::AddrUtils, bus_mod::bus::CpuRAM, ppu_mod::ppu::Ppu};

pub struct IODevice<'a> {
    ram: &'a mut CpuRAM,
    // PPU
    ppu: &'a mut Box<dyn Ppu>, // Cartrige
}

impl IODevice<'_> {
    pub fn new<'a>(ram: &'a mut CpuRAM, ppu: &'a mut Box<dyn Ppu>) -> IODevice<'a> {
        IODevice { ram, ppu }
    }
